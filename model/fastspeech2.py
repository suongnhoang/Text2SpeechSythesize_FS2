...
from transformer import Encoder, Decoder, PostNet, Generator
...
class FastSpeech2(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config):
        ...
        if model_config["mod"]["excitation_generator"]:
    ...
    def forward(
        ...
        output = self.encoder(texts, src_masks, alophone_control)
        ...
        d_rounded,
        mel_lens,
        mel_masks) = self.variance_adaptor( output,
                                            src_masks,
                                            mel_masks,
                                            max_mel_len,
                                            p_targets,
        ...
        acoustics_embedding = pitch_embedding + energy_embedding

        if self.generator is not None:
            excitation_embedding = self.generator(output, mel_masks, acoustics_embedding)
            output = output + excitation_embedding

        output = output + acoustics_embedding
